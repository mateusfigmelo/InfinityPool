...
interface IInfinityPool {
    ...
    function getPoolInfo() external returns (address, address, int256);

    function newLoan(NewLoan.NewLoanParams memory params, bytes calldata data) external returns (uint256, int256, int256);
    function swap(Spot.SwapParams memory params, address receiver, bytes calldata data) external returns (int256 amount0, int256 amount1);
